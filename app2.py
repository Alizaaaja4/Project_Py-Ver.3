...
st.set_page_config(page_title= "COVID-19 Dashboard 2020",
                   ...
                   layout='wide')
# Read CSV File
df = pd.read_csv('covid_19_2020-_Indonesia_.csv')
...
# --- Filter ---
### --- Filter by Country ---
st.sidebar.header("Please Filter the Conntry Here")
...
### --- Filter by Month ---
#### 1. Ubah tanggal -> bulan
df['Date'] = pd.to_datetime(df['Date'])
df['Month'] = df['Date'].dt.month

#### 2. Membuat pilihan bulan di sidebar
month_names = list(calendar.month_name)[1:]  # Mengambil daftar nama bulan dari modul calendar
selected_month = st.sidebar.selectbox(
    "Filter by Month:", 
    month_names)

#### 3. Konversi nama bulan menjadi angka
month_number = list(calendar.month_name).index(selected_month)

#### 4. Filter berdasarkan bulan yang dipilih
